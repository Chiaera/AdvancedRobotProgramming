...
### Collision handling
<div align="center">
  <img src="img/collision.png" width="45%">
</div>
...
---
## Protocol
The client-server protocol used **TPC socket** to implements **sequential handshake** followed by a **cyclic exchange of information** between two drone simulators, composed of the following steps:
1. ### Handshake
...
3. ### Main infinite loop
   ...
   #### Phase A: Server Drone Position
   First the server sends its drone position, preceded by the "drone" message that establishes the message type; the client acknowledges.
   ```
   SERVER → CLIENT: "drone"
   SERVER → CLIENT: "x, y"   (server drone coordinate)       
   CLIENT → SERVER: "dok"   
   ```
   #### Phase B: Client Drone Position (as Server Obstacle)
   Then the server requests one obstacle at a time, anticiped by "obst" message. <br>
   The client responds by sending **its own drone position**, which the server stores as *obstacle*.
   ```
   SERVER → CLIENT: "obst"
   CLIENT → SERVER: "x y"   (the client coordinate)           
   SERVER → CLIENT: "pok"  
   ```
...
To support different orientations between peers, coordinates can be exchanged in a **virtual reference frame**, implemebted through functions `convert_to_virtual()` and `conver_from_virtual`, which take into account the rotation of the coordinate system specified in the `config` file.
...
|------|---------------|-------------|
| **SOLO-PLAYER** | Offline | Local obstacles and targets |
| **SERVER** | Network enabled | Hosts the game,  waits for client connections |
| **CLIENT** | Network enabled | Connects to server, receives world config |

For **client mode**, it is necessary know the *IP address* of the device acting as **server**. 
To get it, open the terminal and run:
``` 
   ip addr
```
Example:
```
   inet 192.168.1.23/24 brd 192.168.1.255 scope

   # In this case, the IP address to insert is:
   # > 192.168.1.23
```
All network service used the defaulto port **8080** (`DAFAULT_PORT`).
